...
def make_retweet_tree(m_id):
    ...
    child_weibo_data = {}
    # 在db.retweet中找这条信息是否转发了其他信息
    # print db.retweet.find_one({'original_mid': m_id})
    # 若是：则它不是祖先节点，在表项中找它的祖先节点
        # 在db.retweet中找它的祖先微博original_mid
    ...
    # 若不是：则它是祖先节点
        ...
        for item in child_weibo_data:
            print child_weibo_data[item], 'kkkk'
...
def preprocess(m_id):
    ...
    text = weibo['text']
    # regExp = rs.compile(r'//@[\u4e00-\u9fa5_a-zA-Z0-9_]+:')
    match = re.search(r'//@[\u4e00-\u9fa5_a-zA-Z0-9_]+:', text)
    if match:
        # print match.group(0)[3:-1], text
        parent_name = match.group(0)[3:-1]
...
make_retweet_tree('3407583433523787')
